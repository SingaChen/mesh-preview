...
export function isFacesRingsJsonName(name) {
  const base = basename(name);
  return /\.json$/i.test(base) && /faces_rings/i.test(base) && !/manifest/i.test(base);
}
...
export function projectFromManifest(data, index, manifestPath = "") {
  ...
  for (const raw of data.outputs) {
    ...
    const facesRingsRef = raw.facesRings || raw.faces_rings;
    const facesRingsFile = facesRingsRef
      ? lookup(index, facesRingsRef, fromDir)
      : findFacesRingsJson(index, meshFile);
...
export function projectFromDiscovery(index) {
  ...
  const outputs = list.map((meshFile) => {
    ...
    return {
      ...
      facesRingsFile: findFacesRingsJson(index, meshFile),
    };
  });
...
function findFacesRingsJson(index, meshFile) {
  const files = (index.jsons || []).filter((f) => isFacesRingsJsonName(f.name));
  if (!files.length) return null;
  return matchOverlay(meshFile, files) || files[0];
}
